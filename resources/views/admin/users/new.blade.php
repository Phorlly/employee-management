<x-admin-layout>
    ...
    <div class="container space-x-4">
        <x-splade-form :default="$data" :action="route('admin.users.store')" class="space-y-2 grid grid-cols-1 md:grid-cols-2 gap-4">
            @csrf
            ...
            <x-splade-input type="password" name="password_confirmation" label="Passwor Confirmation" />
            <x-splade-checkboxes name="roles" label="Roles" :options="$roles" relation inline />
            {{-- <x-splade-radios name="role" label="Roles" :options="$roles" relation inline /> --}}
