                ...
                <div class="flex">
                    ...
                    <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                        <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')">
                            <x-iconsax-bol-home-2 class="w-5 h-5 mx-2" />
                            {{ __('Home') }}
                        </x-nav-link>

                        {{-- @role('admin') --}}
                            <x-nav-link :href="route('admin.index')" :active="request()->routeIs('admin.index')">
                                <x-iconsax-bol-element-2 class="w-5 h-5 mx-2" />
                                {{ __('Dashboard') }}
                            </x-nav-link>
                        {{-- @endrole --}}
                    </div>
                ...
                <div class="hidden sm:flex sm:items-center sm:ml-6">
                    <x-dropdown placement="bottom-end">
                        ...
                        <x-slot name="content" class="shadow-2xl">
                            <x-dropdown-link :href="route('profile.edit')" class="flex flex-row items-center">
                                <x-iconsax-bol-profile-circle class="w-5 h-5 mx-2" />
                                {{ __('Profile') }}
                            </x-dropdown-link>
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                ...
                                <x-dropdown-link as="a" class="flex flex-row items-center" :href="route('logout')"
                                    onclick="event.preventDefault(); this.closest('form').submit();">
                                    <x-iconsax-bro-logout-1 class="w-5 h-5 mx-2" />
                                    {{ __('Log Out') }}
                                </x-dropdown-link>
                            </form>
                        </x-slot>
